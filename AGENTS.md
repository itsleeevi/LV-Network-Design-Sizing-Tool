...
`CableEdgeData.recommendedCrossSection` and `networkRequiredCrossSection` have no Excel counterpart; they are the app's own advice, computed at the end of `runCalculations` and shown together on the wire label. Keep all three cross-section fields distinct:
...
- `networkRequiredCrossSection`: continuous need shown as "Számított". The largest of `requiredCrossSection`, the network voltage-drop need (others at recommended sizes), and `I / maxCurrentDensity` when density is on. This is what the recommendation rounds up from when voltage drop or thermal rules bind; grading can still push the recommendation higher.
...
Settled decisions for the recommendation / Számított pair:

- **"Számított" includes thermal density, but not grading.** A short trunk carrying 28 A can no longer show ~3 mm² next to a large Ajánlott for thermal reasons alone: density raises the continuous figure to `I / density`. A remaining gap to Ajánlott still means grading (or discrete rounding from the cumulative-drop loop) is the driver. Folding grading into Számított was considered and rejected.
